...
if catalog == 'WHSP_Blazars':
  alldata = '/data/user/brelethford/Output/stacking_sensitivity/{0}/{1}_{2}yr/FOM_{3}/{4}/'.format(catalog,version,str(years),fom,llhweight)
else:
  alldata = '/data/user/brelethford/Output/stacking_sensitivity/{0}/{1}_{2}yr/{3}/'.format(catalog,version,str(years),llhweight)
...
hist_bckg,chi_bckg = makehist(bckg)
...
if catalog == 'WHSP_Blazars':
  fig_bckg.savefig('/data/user/brelethford/AGN_Core/Plots/catalog/{0}/TSdist/TSdist_{1}yr_FOM{2}_{3}weight.pdf'.format(catalog,str(years),fom,llhweight))
else:
