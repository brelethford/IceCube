...
def plotretry(datafolder_stack_bin):
    stack_files = [[cache.load(datafolder_stack_bin+file) for file in os.listdir(misc.ensure_dir(datafolder_stack_bin)) if file.endswith('.array')]]
    ##Let's find the declinations used first.
    decfolders = [file for file in os.listdir(datafolder_stack_bin) if file.startswith('dec')]
    dec_stack =  [np.float(decfolder[3:-6]) for decfolder in decfolders]
    flux_stack = [stack_files[0][index]['flux'] for index in range(len(stack_files[0]))]
    inj_stack = [stack_files[0][index]['mu'] for index in range(len(stack_files[0]))]
    sindecrange_stack=np.sin(np.radians(dec_stack))
    results_stack = sorted(zip(sindecrange_stack,inj_stack,flux_stack))
    sindecrange_stack,inj_stack,flux_stack = zip(*results_stack)
    return sindecrange_stack, inj_stack, flux_stack

sindec_retry, inj_retry, flux_retry = plotretry('/data/user/brelethford/Output/stacking_sensitivity/SwiftBAT70m/allsky/sensitivity/')
sindec_retry_cut, inj_retry_cut, flux_retry_cut = plotretry('/data/user/brelethford/Output/stacking_sensitivity/SwiftBAT70m_mc_cut/allsky/sensitivity/')
...
plt.plot(sindec_retry,flux_retry, label = 'retry',color='k')
plt.plot(sindec_retry_cut,flux_retry_cut, label = 'retry with cut',color='orange')
...
fig_sensitivity.savefig('/data/user/brelethford/AGN_Core/Plots/all_sky_test.pdf')
...
plt.plot(sindec_retry,inj_retry, '--', label='retry', color = 'k')
plt.plot(sindec_retry_cut,inj_retry_cut, '--', label='retry with cut', color = 'orange')
...
plt.legend(loc='upper right', prop=propxxsmall)
plt.subplots_adjust (left=.2, bottom=.2)
fig_injected_events.savefig('/data/user/brelethford/AGN_Core/Plots/injected_events_retry.pdf')
