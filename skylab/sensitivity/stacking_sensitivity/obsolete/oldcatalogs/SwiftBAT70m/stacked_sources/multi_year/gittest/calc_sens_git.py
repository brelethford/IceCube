...
### NOTE: Purpose of this particular script variant is to use the new updates to mhuber's stacking skylab code to see how long the new one takes. I'm going to do it all the same, except with the new scripts, and compare to previous values using the regular calculate_sensitivity.py.
...
from bstacking.psLLH import PointSourceLLH 
from bstacking.ps_model import ClassicLLH  
...
if llhweight == 'uniform':
  ...
  llh40= new_data_multi.init40(energy=True, weighting = modelweights['{}'.format(llhweight)],mode='box')
  llh79 = new_data_multi.init79(energy=True, weighting = modelweights['{}'.format(llhweight)],mode='box')
  llh86I= new_data_multi.init86I(energy=True, weighting = modelweights['{}'.format(llhweight)],mode='box')
  llh59= new_data_multi.init59(energy=True, weighting = modelweights['{}'.format(llhweight)],mode='box')
...
#I think I don't need to put src_dec in during the injector phase with this version of the code, but I should CHECK THIS LATER
if injweight == 'uniform':
  inj = PointSourceInjector(Gamma, sinDec_bandwidth=.05,  seed=0)
else:
  inj = PointSourceInjector(Gamma, sinDec_bandwidth=.05,  theo_weight = modelweights['{}'.format(injweight)], seed=0) 

sensitivity = llhmodel.weighted_sensitivity(src_ra=src_ra,src_dec=src_dec,alpha=.5,beta=.9,TSval = TS_beta, inj=inj,eps=0.02, n_iter=250)

sensitivity = llhmodel.weighted_sensitivity(src_ra=src_ra,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,trials={'n_inj':[],'TS':[],'nsources':[],'gamma':[]},longrun=opts.longrun,bckg_trials=bckg_trials,eps=0.02,n_iter=250)
print sensitivity
