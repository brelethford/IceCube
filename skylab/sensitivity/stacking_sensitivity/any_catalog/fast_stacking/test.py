...
if cat:
  sens_dir = misc.ensure_dir (outputfolder+'stacking_sensitivity/{0}yr/{1}/{2}/'.format(len(seasons),cat,weight))
else:
...
if cat:
    dir_name = "/data/user/brelethford/AGN_Core/Plots/fast/plots/stacking/{}yr/{}/{}/".format(len(seasons),cat,weight)
...
def get_test_mu2flux():
  result = inj.mu2flux(1)
  return result

outfile = sens_dir + 'mu2flux_test.array'
result = cache.get(outfile, get_test_mu2flux)

print ("mu2flux test yields: {}".format(result*1e3))
###############################################################################
