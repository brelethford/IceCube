...
llh79 = mikebins.init79(energy=True)
llh86I= mikebins.init86I(energy=True)
llh59= mikebins.init59(energy=True)
llh40= mikebins.init40(energy=True)
...
llhmodel = mikebins.multi_init(samples,energy=True)
...
for i in range(len(src_dec)):
  inj = inj_array[i]
  result =  StackingMultiPointSourceLLH.weighted_sensitivity(llhmodel,src_ra=np.atleast_1d(0.),src_dec=np.atleast_1d(src_dec[i]),alpha=.5,beta=.9,inj=inj,trials={'n_inj':[],'TS':[],'nsources':[],'gamma':[]},bckg_trials=bckg_trials,eps=0.04,n_iter=20)
