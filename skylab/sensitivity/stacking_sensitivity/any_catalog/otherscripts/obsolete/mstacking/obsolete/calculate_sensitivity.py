...
#Get sirin variable as a boolean
if sirin:
  sirin=True
else:
  sirin=False
...
#TSval = 0
TSval = cache.get(backfolder + 'TSval.pickle', getTSval)
...
if catalog == 'WHSP_Blazars' or mhubertest:
  ...
  llhmodel = datascript.load7yr_mese(sirin=sirin)
  MC = datascript.loadMC7yr_mese(sirin=sirin)
else:    
  if catalog == '4yr_Starburst' and years <= 4:
...
sensitivity = StackingMultiPointSourceLLH.weighted_sensitivity(llhmodel,src_ra=src_ra,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,mc=MC,TSval = TSval,w_theoMC=modelweights['{}'.format(llhweight)],w_theo=modelweights['{}'.format(injweight)],eps=0.025,n_iter=100)
print sensitivity
...
if mhubertest:
  out_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/{0}/mstacking_{1}yr/{2}_mhubertest/{3}_inj/sensitivity/'.format(catalog,str(years),llhweight,injweight))
if catalog == 'WHSP_Blazars':
  if sirin:
    sens_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/{0}/mstacking_{1}yr/FOM_{2}/{3}_sirin/{4}_inj/sensitivity/'.format(catalog,str(years),llhweight,str(fom),injweight))
  else:
    sens_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/{0}/mstacking_{1}yr/FOM_{2}/{3}/{4}_inj/sensitivity/'.format(catalog,str(years),llhweight,str(fom),injweight))
else:
  sens_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/{0}/mstacking_{1}yr/{2}/{3}_inj/sensitivity/'.format(catalog,str(years),llhweight,injweight))
