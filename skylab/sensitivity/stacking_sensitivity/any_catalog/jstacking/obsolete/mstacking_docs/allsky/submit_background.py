...
for i in range(batches):
  for sindec,deg_dec in zip(sindecrange,degdecrange):
    commands.append('{0} {1} --batch {2} --batchsize {3} --sindec {4} --years {5} --sirin {6} --mhubertest {7} --mese {8}'.format (env_shell, job_script, i, batchsize, sindec, str(n), sirin, mhubertest, mese))
