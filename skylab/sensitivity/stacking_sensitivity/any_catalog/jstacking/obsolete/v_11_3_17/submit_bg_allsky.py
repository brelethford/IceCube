...
for i in range(batches):
  for sindec,deg_dec in zip(sindecrange,degdecrange):
    commands.append('{0} {1} --batch {2} --batchsize {3} --sindec {4} --n {5} --sirin {6} --mese {7} --datatype {8}'.format (env_shell, job_script, i, batchsize, sindec, str(n), sirin, mese, datatype))
