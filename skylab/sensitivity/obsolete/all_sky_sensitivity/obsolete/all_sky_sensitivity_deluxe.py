...
def sensitivity_skylab(Gamma=2,src_dec=0, n_iter=1000, n_bckg=100000, eps=0.005):
  ...
  return PointSourceLLH.weighted_sensitivity(llh,src_ra=0.0,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,mc=mc,n_iter=n_iter, n_bckg=n_bckg, eps=eps)
...
#Now we do the work...
sensitivity_array = sensitivity_skylab(src_dec=opts.dec, n_iter=2000, n_bckg=200000, eps=0.003)

#choose an output dir, and make sure it exists
this_dir = os.path.dirname(os.path.abspath (__file__))
## NOTE: THE OUT DIRECTORY CHANGES DEPENDING ON WHICH SUBMITTER I'M TESTING!:
out_dir = misc.ensure_dir ('/data/user/brelethford/Output/all_sky_sensitivity/results/deluxe/')
