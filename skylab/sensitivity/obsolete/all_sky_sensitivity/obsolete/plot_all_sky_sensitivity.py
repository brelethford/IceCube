...
##I'll convert the flux for gamma=1,3.
flux_gamma1 = np.multiply(flux_gamma1,100.)
flux_gamma3 = np.divide(flux_gamma3,100.)
...
fig_sensitivity.savefig('/data/user/brelethford/AGN_Core/Plots/single_source_sensitivity.png')
...
#Different Gamma Sensitivities: Note, we have to convert from 1 TeV before plotting here.
...
ax.set_xlabel(r'$\sin{(\delta)}$')
ax.set_ylabel(r'$E^2 \cdot \left( \frac{E}{100 \textrm{\small{TeV}}} \right)  ^{\gamma -2} \frac{\partial N}{\partial E} [\frac{TeV}{cm^2 s}]$')
ax.set_xlim(-1.,1.)
#ax.set_ylim(1e-12,1e-10)
ax.semilogy()
ax.grid() 
...
fig_gamma.savefig('/data/user/brelethford/AGN_Core/Plots/gamma_sensitivity.png')
...
fig_disc.savefig('/data/user/brelethford/AGN_Core/Plots/single_source_discovery_potential.png')
