...
def sensitivity_skylab(Gamma=2,src_dec=0, n_iter=1000, n_bckg=10000, TSval=0):
  inj = PointSourceInjector(Gamma, sinDec_bandwidth=.1, src_dec= src_dec,seed=0)
  return PointSourceLLH.weighted_sensitivity(llh,src_ra=0.0,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,mc=mc,n_iter=n_iter,TSval=TSval)
...
#Now we do the work...
sensitivity_array = sensitivity_skylab(src_dec=np.arcsin(opts.dec))

#choose an output dir, and make sure it exists
this_dir = os.path.dirname(os.path.abspath (__file__))
## NOTE: THE OUT DIRECTORY CHANGES DEPENDING ON WHICH SUBMITTER I'M TESTING!:
out_dir = misc.ensure_dir ('/data/user/brelethford/Output/all_sky_sensitivity/results/box/')
