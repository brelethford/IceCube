...
class Csky6yr (Timed, Driver):
    ...
    def run (self, arglist=[]):
        ...
        parser.add_option ('-c', '--conf-dirs', dest='conf_dirs',
                default=os.path.abspath ('conf_corrected_stefan'), metavar='DIRS',
    ...
    def datasets (self):
        try:
            ...
            dataset2012 = loading ('{0}/IC86II.dataset'.format (self.mode_dir))
            dataset2013 = loading ('{0}/IC86III.dataset'.format (self.mode_dir))
            dataset2014 = loading ('{0}/IC86IV.dataset'.format (self.mode_dir))
    ...
    def build_cdata (self):
        ...
        ids_exp = ['{}'.format (s)
               for s in 'IC40 IC59 IC79b IC86_noMESE IC86-2012_noMESE IC86-2013_noMESE IC86-2014_noMESE MESE MESE_followup'.split()]
        ...
        #       for s in 'IC40 IC59 IC79 IC86I epinat_3yr'.split()]
        ids_mc = ['{}'.format (s)
               for s in 'IC40 IC59 IC79b_noMESE IC86_noMESE IC86-2012_noMESE MESE'.split()]
        ...
        mu2012 = cdata.mu2012 = Arrays ()
        mu2013 = cdata.mu2013 = Arrays ()
        mu2014 = cdata.mu2014 = Arrays ()
        muMESE_first = Arrays ()
        ...
        muMESE = cdata.muMESE = combined_arrays((muMESE_first,muMESEfollowup))
    ...
    def setup_datasets (self):
        ...
        saving (dataset86II, '{}/IC86II.dataset'.format (self.mode_dir))
        saving (dataset86III, '{}/IC86III.dataset'.format (self.mode_dir))
        saving (dataset86IV, '{}/IC86IV.dataset'.format (self.mode_dir))
