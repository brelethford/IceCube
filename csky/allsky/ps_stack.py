...
from icecube.csky import ana, pdf, hyp, llh, trial, bk, cat, coord, dists, selections
...
class Csky6yr (Timed, Driver):
    ...
    def run (self, arglist=[]):
        ...
        parser.add_option ('-c', '--conf-dirs', dest='conf_dirs',
                default=os.path.abspath ('conf_4yr_b'), metavar='DIRS',
    ...
    @property
    def repo_dir (self):
        return ensure_dir('{}/data/analyses'.format (self.root_dir))
    ...
    def psdata (self):
        try:
          ...
          elif self.conf.root.data == 'npz': #whose data is being used? mine or stefan's?
          ...
          elif self.conf.root.data == 'my': #whose data is being used? mine or stefan's? 
            if self.conf.root.pullfactor:
          ...
          elif self.conf.root.data == 'standard': #whose data is being used? mine or stefan's?

            if self.conf.root.pullfactor:
                datasetfolder = self.root_dir+'/data/datasets_standard'
                print ("Standard data, included pullfactor")
            else:
                datasetfolder = self.root_dir+'/data/datasets_nopullfactor_standard'
                print ("Standard data, no pullfactor")
    ...
    def datasets (self):
        try:
            ...
            if self.conf.root.data == 'npz': #whose data is being used? mine or stefan's?
              if 'corrected' in self.mode.words[1]:
                if self.conf.root.pullfactor:
                  datasetfolder = self.root_dir+'/data/datasets_corrected'
                  print ("For 'corrected' npz files")
                else:
    ...
    def build_data (self):
        """
        Build elisa's fixed data. This is from `http://code.icecube.wisc.edu/projects/icecube/browser/IceCube/sandbox/skylab/trunk/skylab/datasets/PointSourceTracks_v002p01.py`.
        """
        psdata = Vars ()
        #Is pullcorrected data used?
        pullfactor = self.conf.root.pullfactor
        ...
        ids_exp = ['{}'.format (s)
               for s in 'IC40_exp IC59_exp IC79b_exp IC86_exp IC86-2012_exp_v2 IC86-2013_exp_v2 IC86-2014_exp_v2'.split()]
        filenames_exp = ['{}/data/elisafixeddata/{}.npy'.format (self.root_dir, i)
                     for i in ids_exp]

        #add MESE from stefan's dataset
        ids_exp_MESE = ['{}'.format (s)
               for s in 'MESE MESE_followup'.split()]
        filenames_exp_MESE = ['{}/data/stefandata/{}_exp.npy'.format (self.root_dir, i)
                     for i in ids_exp_MESE]

        filenames_exp.append(filenames_exp_MESE[0])
        filenames_exp.append(filenames_exp_MESE[1])

        ids_mc = ['{}'.format (s)
               for s in 'IC40_corrected_MC IC59_corrected_MC IC79b_corrected_MC IC86_corrected_MC IC86-2012_corrected_MC_v2'.split()]

        filenames_mc = ['{}/data/elisafixeddata/{}.npy'.format (self.root_dir, i)
                     for i in ids_mc]

        #add MESE from stefan's dataset
        ids_mc_MESE = ['MESE']

        filenames_mc_MESE = ['{}/data/stefandata/{}_MC.npy'.format (self.root_dir, i)
                     for i in ids_mc_MESE]


        filenames_mc.append(filenames_mc_MESE[0])
        ...
        mus = [mu40,mu59,mu79,mu86,mu2012,muMESE_first,muMESEfollowup] 
        ...
        mus = [mu40,mu59,mu79,mu86,mu2012,muMESE] 
        ...
        #I'll define IC86III and IV to be the same as II - only livetime is different.
        mu2013 = psdata.mu2013 = mu2012
        mu2014 = psdata.mu2014 = mu2012 
        ...
        saving (psdata, '{0}/psdata.vars'.format (misc.ensure_dir(datasetfolder)))
    # setting up elisa's npz data. NOTE - no sirin!
    '''
    @command
    def build_data (self):
        """
        Build elisa's npz data.
        """
        psdata = Vars ()
        #Is pullcorrected data used?
        pullfactor = self.conf.root.pullfactor
        ...
        ids_exp = ['{}'.format (s)
               for s in 'IC40 IC59 IC79 IC86-I IC86-II-III-IV'.split()]
        filenames_exp = ['{}/data/elisadata/{}_data_pull_corrected.npy'.format (self.root_dir, i)
                     for i in ids_exp]
        ...
        ids_mc = ['{}'.format (s)
               for s in 'IC40 IC59 IC79 IC86-I IC86-II'.split()]

        filenames_mc = ['{}/data/elisadata/{}_mc_pull_corrected.npy'.format (self.root_dir, i)
                     for i in ids_mc]
    ...
    def setup_datasets (self):
        ...
        if 'corrected' in self.mode.words[0]:
          ...
          if self.conf.root.pullfactor:
            ...
            datasetfolder = self.root_dir+'/data/my_datasets_nopullfactor/'
            print ("My data, no pullfactor")
        elif self.conf.root.data == 'standard':
          if self.conf.root.pullfactor:
            datasetfolder = self.root_dir+'/data/datasets_standard/'
            print ("Standard data, included pullfactor")
          else:
        ...
        if not self.conf.root.data == 'standard':
        ...
        if self.conf.root.data == 'npz' or self.conf.root.data == 'standard':
    ...
    def setup_ana (self):
       ...
       """
       repo = selections.Repository (self.repo_dir, username='brelethford')

       if 'yrs4' in self.mode.words[1]:
           analysis = selections.get_analysis (repo, selections.PSDataSpecs.ps_4yr)
       if 'yrs7' in self.mode.words[1]:
           analysis = selections.get_analysis (repo, selections.PSDataSpecs.ps_7yr)
       if 't7g3' in self.mode.words[1]:
           analysis = selections.get_analysis (
               repo, selections.PSDataSpecs.ps_7yr + selections.GFUDataSpecs.gfu_3yr,
           )
       if 'mesc' in self.mode.words[1]:
           analysis = selections.get_analysis (repo, selections.MESEDataSpecs.mesc_7yr)
    ...
    def energypdf_compare (self):
        ...
        size = len(samples) - len(samples)%4
        for g in np.linspace(1,4,4):#analysis.gammas:
          fig, ax = pfig(aspect=size/3., width = 7.*size/4)
          ax.cla()
          gs = mpl.gridspec.GridSpec (2, size/2, hspace=.3, wspace = .2)
          plot_dir  = misc.ensure_dir ('{0}/plots/energypdfs/compare{1}yr'.format (self.mode_dir,self.conf.root.n_year))
          #csky - ratio
          for n,sample in zip(range(len(samples)),samples):
            ax = plt.subplot (gs[n])
            ratio =  self.analysis.pdf_ratios_energy[sample].epdfratios[g]
            if sample == 'MESE':
              hsb = histlite.hist_from_function(([-1,0],
                                               np.linspace(ratio.logenergy_range[0],ratio.logenergy_range[1],400), 
                                               np.linspace(ratio.dist_range[0],ratio.dist_range[1],400,)),
                                               ratio)[-1,:,:].T
              ax.set_xlim(0.,500.)
              xlab = r'starting distance $[\mathrm{m}]$'
            else:
              hsb = histlite.hist_from_function((np.linspace(ratio.sindec_range[0],ratio.sindec_range[1],400,),
                                               np.linspace(ratio.logenergy_range[0],ratio.logenergy_range[1],400), [0,1]),
                                               ratio)[:,:,0]
              ax.set_xlim(-1.,1.)
              xlab = r'$\sin{\delta}]$'
            histlite.plot2d (ax, hsb, cmap='RdBu_r', log=True, vmin=1e-2, vmax=1e2, cbar=True)
            ax.set_ylim(1,10)
            ylab = r'$log(E[\mathrm{GeV}])$'
          ...
          '''
          #csky - bg
          gs1 = mpl.gridspec.GridSpec (2, size/2, hspace=.3, wspace = .2)
          fig, ax = pfig(aspect=size/3., width = 7.*size/4)
          ax.cla()
          for n,sample in zip(range(len(samples)),samples):
            ax = plt.subplot (gs1[n])
            if sample == 'MESE':
              hb = self.analysis.pdf_ratios_energy[sample].epdfratios[g].h_bg_0[-1,:,:]
              ax.set_xlim(0.,500.)
              xlab = r'starting distance $[\mathrm{m}]$'
            else:
              ...
              ax.set_xlim(-1.,1.)
              xlab = r'$\sin{\delta}]$'
            if self.conf.root.cap_thresh > 0:
               #need a line for 68% containment?
               hb_c = hb.contain(1,frac=self.conf.root.cap_thresh) #1 for energy axis
          ...
          #csky - sig
          gs2 = mpl.gridspec.GridSpec (2, size/2, hspace=.3, wspace = .2)
          fig, ax = pfig(aspect=size/3., width = 7.*size/4)
          ax.cla()
          for n,sample in zip(range(len(samples)),samples):
            ax = plt.subplot (gs2[n])
            if sample == 'MESE':
              hb = self.analysis.pdf_ratios_energy[sample].epdfratios[g].h_sig_0[-1,:,:]
              ax.set_xlim(0.,500.)
              xlab = r'starting distance $[\mathrm{m}]$'
            else:
              ...
              ax.set_xlim(-1.,1.)
              xlab = r'$\sin{\delta}]$'
            histlite.plot2d (ax, hb, cmap='OrRd_r', log=True, cbar=True)#vmin=1e-1, vmax=1e1, cbar=True)
            ax.set_ylim(1,10)
            ylab = r'$log(E[\mathrm{GeV}])$'
            ax.set_title('{}'.format(sample))
    ...
    def do_bg_trials (self):
        ...
        analysis=self.analysis
        #Include to test space-only distributions.
        try:
          noE = self.conf.root.noE 
        except:
          noE = False
        if noE:
          analysis.set_energy_pdfs (disable=True) 
          print ("Doing space-only")
        np.random.seed (seed)
        data = hyp.DataHypothesis (analysis)
        gammas = np.linspace(1.,4.,13)
    ...
    def do_n_sig (self):
        ...
        if self.opts.cat:
          self.analysis.ps_sindec_width = np.radians(2)
          bg_tsd = self.bg_tsds
        else:
          self.analysis.ps_sindec_width = np.radians(2)
        ...
        ps = hyp.PointSourceHypothesis (self.analysis, dec, ra, gamma, weights = weights,
                                        ...
                                        sigsub = self.conf.root.sigsub, can_inject = True)
        ...
        sens = tr.get_sens_batches (np.float(ts), beta, tol = 0.3, min_batch_size = 100)
        flux100TeV = ps.to_flux (sens['n_sig'], 100, 1e3) # E0^2 Phi(E0) in TeV/cm^2/s
        fluxGeV = ps.to_flux (sens['n_sig']) # E0^2 Phi(E0) in TeV/cm^2/s
    ...
    @command
    def submit_ns_gamma (self):
        """
        Submit jobs for plotting ns_gamma over whole sky.
        """
        job_root = self.conf.root.cluster_job_dir
        job_dir = '{0}/plot_ns_gamma/{1}'.format (job_root, job_id)
        submitter = Submitter (job_dir=job_dir,max_jobs = 100, memory=4)
        commands, labels = [], []
        ...
        test_ext = src_ext
        dec_degs = np.concatenate([np.arange (-90, 90, 12), np.atleast_1d(0)])
        #dec_degs = np.concatenate([np.linspace(-90.,-6.,29), np.linspace(-5.,5.,11), np.linspace(6,90,29)])
        for dec_deg in dec_degs:
            dec_deg = max (-89, min (89, dec_deg))
            command = '{} {} ns_gamma_fit --conf-dirs={} ' \
                    ' --n-trials={}' \
                    ' --n-inj={}' \
                    ' --test-dec={:+07.2f}' \
                    ' --test-ext={:04.1f}' \
    ...
    @command
    def ns_gamma_fit (self):
        """
        Using bg trials, plot many ns vs. gammas on a plot
        """
        ...
        #set up / start plot
        plot_dir  = misc.ensure_dir ('{0}/plots/fit_trials/sig_int_trials/{1}'.format (self.mode_dir,dec_deg))

        fig, ax = pfig()

        ax.set_title("ns vs. gamma - dec {1}, {2} trials".format(self.conf.root.n_year, dec_deg, n_trials))
        ax.set_xlim (0, 40)
        ax.set_ylim (1, 4)
        ax.set_xlabel('ns')
        ax.set_ylabel('gamma')

        #Create many trials for each gamma
        if n_inj > 0:
          fit_gammas = [2.,3.]
        else:
          fit_gammas = [2.]
        colors = ['red','blue']
        for gamma, color in zip(fit_gammas,colors):
          ps = hyp.PointSourceHypothesis (self.analysis, dec, ra, gamma, extensions = test_ext, weights = weights, sigsub = self.conf.root.sigsub)
          tr = trial.PSTrialRunner (data, ps, ps.tests (gammas), cut_n_sigma=np.inf)
          c = tr.get_many_ts_ns_gamma (n_trials, n_inj=n_inj)
          result_dict = {'TS':c[0],'ns':c[1],'gamma':c[2]}
          sm = bk.SavingModel (
                'ts',
                'many_ts_n_inj_{}.dict'.format(n_inj),
                )
          filename = sm.save (
                 result_dict, '{0}/fit_trials/{1}/many_ts_n_inj_{2}_gamma_{3}'.format (self.mode_dir,dec_deg,n_inj,gamma))
          x = result_dict['ns']
          y = result_dict['gamma']
          #plot:
          if n_inj > 0:
            plt.scatter(x, y, color=color, alpha=0.5, label = r'$\gamma = {}$'.format(gamma)) 
            plt.hlines(gamma, 0., 40., color=color, alpha = 0.5)
            plt.vlines(n_inj, 1., 4., color=color, alpha = 0.5)
            #h = histlite.hist((x,y),bins = (40,12), range = ((0,40),(1,4)))
            #histlite.plot2d (ax_h, h, cmap='RdBu_r', cbar=True)
          else:
        ...
        plt.legend(loc='upper right', prop=propsmall)
        if n_inj > 0:
          savingfig (fig, plot_dir, 'n_inj__{}'.format(n_inj))  
        else:
          savingfig (fig, plot_dir, 'bg')  
        plt.close(fig)        

    @command
    def submit_plot_ns_gamma (self):
        """
        Submit jobs for plotting ns_gamma over whole sky.
        """
        job_root = self.conf.root.cluster_job_dir
        ...
        for dec_deg in dec_degs:
            dec_deg = max (-89, min (89, dec_deg))
            command = '{} {} plot_ns_gamma_fit --conf-dirs={} ' \
                    ' --n-trials={}' \
                    ' --n-inj={}' \
                    ' --test-dec={:+07.2f}' \
                    ' --test-ext={:04.1f}' \
    ...
    def plot_ns_gamma_fit (self):
        ...
        colors = ['red','blue']
        for gamma, color in zip(fit_gammas,colors):
          result_dict = cache.load('{0}/fit_trials/{1}/many_ts_n_inj_{2}_gamma_{3}/many_ts_n_inj_{4}.dict'.format (self.mode_dir,dec_deg,n_inj,gamma,n_inj))
          x = result_dict['ns']
          y = result_dict['gamma']
          #plot:
          fig, ax = pfig()
          ...
          plt.legend(loc='upper right', prop=propsmall)
          if n_inj > 0:
            savingfig (fig, plot_dir, 'n_inj__{}_gamma_{}'.format(n_inj,gamma))  
          else:
            savingfig (fig, plot_dir, 'bg')  
          plt.close(fig)        
          #plot hist:
          ...
          if n_inj > 0:
            ...
            plt.vlines(n_inj, 1., 4., color=color, alpha = 0.5)
            h = histlite.hist((x,y),bins = (40,12), range = ((0,40),(1,4)))
            histlite.plot2d (ax, h, cmap='RdBu_r', cbar=True)
          else:
            h = histlite.hist((x,y),bins = (40,12), range = ((0,40),(1,4)))
            histlite.plot2d (ax, h, cmap='RdBu_r', cbar=True)
          plt.legend(loc='upper right', prop=propsmall)
          if n_inj > 0:
            savingfig (fig, plot_dir, 'n_inj__{}_gamma_{}_hist'.format(n_inj,gamma))  
          else:
            savingfig (fig, plot_dir, 'bg_hist')  
          plt.close(fig)        
    ...
    def print_sens (self):
        ...
        if self.opts.cat:
            ...
            sensdir = self.mode_dir + '/cats/{0}/{1}/'.format(self.opts.cat,weights)
            sig_int_cat = cache.load(sensdir+'sig_int.dict')
            sens_ns = sig_int_cat[0][.9][2][2]['+000inf'][0]
            disc_ns = sig_int_cat[5][.5][2][2]['+000inf'][0]
            sens_cat = sig_int_cat[0][.9][2][2]['+000inf'][2]
            disc_cat = sig_int_cat[5][.5][2][2]['+000inf'][2]
            print ("Sensitivity: " +str(sens_cat) + ", ns = " + str(sens_ns))
            print ("Discovery Potential: " +str(disc_cat) + ", ns = " + str(disc_ns))

    @command
    def sens (self):
        """Plot allsky integrated sensitivity, possibly w/ the catalog sensdisc."""
        ...
        label2 = 'csky / $E^{{-{0}}}${1}'.format (
                #int (self.conf.root.n_year), gamma, cutoff_str)
                2, cutoff_str)
        sig_int = cache.load(self.mode_dir+'/sig_int.dict')
        #sensdir = self.mode_dir + '/sig_int/'
        #sig_int = bk.get_all(
        #        sensdir,'sens.pickle', lambda x:x[0])
        ...
        #optional: make it so we're only looking at every 3 degrees.
        #set1 = np.array(x.keys())
        #set2 = np.arange(-84.,90.,3)
        #x = {key:x[key] for key in set1[np.in1d(set1,set2)]}
        sindec2 = np.array ([
            np.sin (np.radians(k)) for k in sorted (x)])
        if 'yrs7' in self.mode.words[1] or 'SNR' in self.mode.words:
          sys = self.opts.sys_total
        elif 'IC86' in self.mode.words[1]:
          sys = 1.
        else:
          sys = 1.21
        sens2 = sys*np.array ([
        ...
        if 'oneyear' in self.mode.words:
        ...
        elif 'IC86II' in self.mode.words[1]:
          ...
          thing_sens = np.genfromtxt('{0}/data/sens_86.csv'.format (self.root_dir), delimiter = ',')
          thing_disc = np.genfromtxt('{0}/data/disc_86.csv'.format (self.root_dir), delimiter = ',')
        elif 'yrs4' in self.mode.words[1]:
        ...
        elif 'test' in self.mode.words[1]:
          thing_sens = np.genfromtxt('{0}/data/sens4yr.csv'.format (self.root_dir), delimiter = ',')
          thing_disc = np.genfromtxt('{0}/data/disc4yr.csv'.format (self.root_dir), delimiter = ',')
        ...
        lw = 2
        if 'yrs7' in self.mode.words[1] or 'SNR' in self.mode.words:
          sd, s2, s3 = np.sin(thing_sens['dec']), thing_sens['2'], thing_sens['3']
          dd, d2, d3 = np.sin(thing_disc['dec']), thing_disc['2'], thing_disc['3']
        else:
          sd, s2 = thing_sens.T
          dd, d2 = thing_disc.T
        ...
        #savingfig (fig, plot_dir, 'MESC_vs_track_sensitivity')
        if ext !=0.0:
          savingfig (fig, plot_dir, '{}_sensitivity_ext_{}'.format(self.mode.words[1],ext))
        else:
          savingfig (fig, plot_dir, '{}_sensitivity'.format(self.mode.words[1]))
    ...
    def unblind_sources_test (self):
        ...
        print ('Complete!')
        out_dir  = misc.ensure_dir ('{0}/cats/{1}/{2}/unblinding_test'.format (self.mode_dir,self.opts.cat,self.opts.weights))
        if dec_order:
          if self.opts.ns:
          ...
          saving (results, out_dir+'/batch_{}_results.pickle'.format(self.opts.i_source))
    ...
    def plot_pdf_ratio (self):
        ...
        for gamma in gammas:
          ...
          for sample in samples:
            ...
            #now skylab: Use the premade llh 
            llh = llhs[sample]
	    exp = llh.exp
            mc = llh.mc
