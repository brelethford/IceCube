...
class Csky6yr (Timed, Driver):
    ...
    def run (self, arglist=[]):
        ...
        parser.add_option ('-c', '--conf-dirs', dest='conf_dirs',
                default=os.path.abspath ('conf_stefan'), metavar='DIRS',
    ...
    def do_n_sig (self):
        ...
        if self.opts.cat:
          ...
          bg_tsd = self.bg_tsds[self.opts.cat][self.conf.root.n_year]
        else:
    ...
    def sig_plots (self):
        ...
        if self.opts.cat:
            ...
            fig, ax = pfig()
            bg_tsd = bg_tsds[self.opts.cat][self.conf.root.n_year]
            x = np.linspace(1e-3,20,100)
