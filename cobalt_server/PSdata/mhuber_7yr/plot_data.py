...
plotfolder = '/data/user/brelethford/AGN_Core/Plots/data/mhuber_7yr/'
...
for m,i,l,c in zip(mc,exp,labels,colors):
  ...
  hexp = histlite.Hist.normalize(histlite.hist(np.degrees(i['sigma']), bins=bins, range = (1e-2,1e3), log=True))
  histlite.plot1d(ax,hexp,histtype='step',label=l, color = c)
  hmc = histlite.Hist.normalize(histlite.hist(np.degrees(mc[m]['sigma']), bins=bins, range = (1e-2,1e3), weights = weight_mc, log=True))
  histlite.plot1d(ax,hmc,histtype='step', color = c,linestyle = ':')
