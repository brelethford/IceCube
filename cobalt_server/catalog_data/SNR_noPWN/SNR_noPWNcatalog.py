...
filename =open('/home/brelethford/Documents/IceCube_Research/Scripts/AGN_Core/catalog_data/SNR_noPWN/rawparams.txt','r')
...
for i in table[1:]:
  name.append(i[0])
  ra.append(np.radians(float(i[1])))
  dec.append(np.radians(float(i[2])))
  weight.append(float(i[3]))
  extension.append(float(i[4]))

#Finally let's save these values.

params = {'name':name,'ra':ra,'dec':dec,'weight':weight,'extension':np.radians(extension)}

cache.save(params, '/data/user/brelethford/Data/SNR_noPWN/pickle/params.pickle')
