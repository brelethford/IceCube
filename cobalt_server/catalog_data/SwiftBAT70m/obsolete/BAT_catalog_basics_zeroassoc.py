...
data=table[1:]
datatype=[]
#Below, I've included a condition that the assoc parameter is 0 (means we have confirmed the x-ray source with a separate catalogue)
for i in range(len(data)):
	try:
		if data[i][2].split(',')[-3]=='0':
...
ax.set_title('Swift BAT 70-Month Catalogue - Zero-Assoc Most Frequent Sources')
plt.xticks(ypos,typedict.keys(), rotation='vertical')
plt.bar(ypos, typedict.values(), align="center")
ax.text (4, 120, 'Total # of sources = ' + str(total), **paramhargs)

plt.subplots_adjust(bottom=0.2)
figtype.savefig("/home/relethford/Documents/IceCube_Research/Plots/AGNCore/X-Ray_Catalogue/SwiftBAT_70M/sourcetype_assoc.pdf")
...
ax.set_ylabel('Frequency in Catalogue')
ax.set_title('Swift BAT 70-Month Catalogue - Zero-Assoc Seyfert Sources')
plt.xticks(ypos,typedict_sy.keys(), rotation='vertical')
plt.bar(ypos, typedict_sy.values(), align="center")
ax.text (4, 120, 'Total # of sources = ' + str(total), **paramhargs)
plt.subplots_adjust(bottom=0.2)
figseyfert.savefig("/home/relethford/Documents/IceCube_Research/Plots/AGNCore/X-Ray_Catalogue/SwiftBAT_70M/seyfert_sources_assoc.pdf")
...
ax.set_xlabel('Seyfert Type')
ax.set_ylabel('Frequency in Catalogue')
ax.set_title('Swift BAT 70-Month Catalogue - Zero-Assoc Numerous Seyfert Sources')
plt.xticks(ypos,typedict_big_sy.keys(), rotation='vertical')
plt.bar(ypos, typedict_big_sy.values(), align="center")
ax.text (2.5, 120, 'Total # of sources = ' + str(total), **paramhargs)
plt.subplots_adjust(bottom=0.2)
figbigseyfert.savefig("/home/relethford/Documents/IceCube_Research/Plots/AGNCore/X-Ray_Catalogue/SwiftBAT_70M/big_seyfert_sources_assoc.pdf")
