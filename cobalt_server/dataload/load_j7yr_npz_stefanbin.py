...
#Loading Zone#
filename_pickle = '/home/brelethford/Documents/IceCube_Research/Scripts/AGN_Core/data/pickle/npz/'
...
def init40(mode='box', nopull=False, **kwargs):
    ...
    llh_model = EnergyLLH(energy_bins,
                          sinDec_bins=dec_bins, allow_empty=True, kernel = 1, seed = 2)        
...
def init79_sirin(decorr=False, nopull=False, mode='box', **kwargs):
    if decorr:
      ...
      arr_exp = np.load(filename_pickle+"nopull/IC79_exp.npy")
      arr_mc = np.load(filename_pickle+"nopull/IC79_corrected_MC.npy")
    else:
      arr_exp = np.load(filename_pickle+"standard/IC79_exp.npy")
      arr_mc = np.load(filename_pickle+"standard/IC79_corrected_MC.npy")
...
def init86II(decorr=False, nopull=False, mode='box', **kwargs):
    if decorr:
      arr_exp = cache.load(filename_pickle+"IC86II/noMESE/exp.pickle")
      arr_mc = cache.load(filename_pickle+"IC86II/noMESE/mc.pickle")
    elif nopull:
      arr_exp = np.load(filename_pickle+"nopull/IC86_exp.npy")
      arr_mc = np.load(filename_pickle+"nopull/IC86_corrected_MC.npy")
    else:
      arr_exp = np.load(filename_pickle+"standard/IC86-2012_noMESE_exp.npy")
      arr_mc = np.load(filename_pickle+"standard/IC86-2012_noMESE_corrected_MC.npy")
      dec_bins = np.unique(np.concatenate([
    ...
    llh = PointSourceLLH(arr_exp, arr_mc, livetime=livetime_IC86II, llh_model=llh_model,
                         mode=mode, 
                         **kwargs)
...
def init86III(decorr=False, nopull=False, mode='box', **kwargs):
    if decorr:
      arr_exp = cache.load(filename_pickle+"IC86III/noMESE/exp.pickle")
      arr_mc = cache.load(filename_pickle+"IC86III/noMESE/mc.pickle")
    elif nopull:
      arr_exp = np.load(filename_pickle+"nopull/IC86_exp.npy")
      arr_mc = np.load(filename_pickle+"nopull/IC86_corrected_MC.npy")
    else:
      arr_exp = np.load(filename_pickle+"standard/IC86-2013_noMESE_exp.npy")
      arr_mc = np.load(filename_pickle+"standard/IC86-2012_noMESE_corrected_MC.npy")
      dec_bins = np.unique(np.concatenate([
                             np.linspace(-1., -0.93, 4 + 1),
                             np.linspace(-0.93, -0.3, 10 + 1),
    ...
    llh = PointSourceLLH(arr_exp, arr_mc, livetime=livetime_IC86III, llh_model=llh_model,
                         mode=mode, 
                         **kwargs)
...
def init86IV(decorr=False, nopull=False, mode='box', **kwargs):
    if decorr:
      arr_exp = cache.load(filename_pickle+"IC86III/noMESE/exp.pickle")
      arr_mc = cache.load(filename_pickle+"IC86III/noMESE/mc.pickle")
    elif nopull:
      ...
      arr_exp = np.load(filename_pickle+"standard/IC86-2014_noMESE_exp.npy")
      arr_mc = np.load(filename_pickle+"standard/IC86-2012_noMESE_corrected_MC.npy")
      dec_bins = np.unique(np.concatenate([
                             np.linspace(-1., -0.93, 4 + 1),
                             np.linspace(-0.93, -0.3, 10 + 1),
    ...
    llh = PointSourceLLH(arr_exp, arr_mc, livetime=livetime_IC86IV, llh_model=llh_model,
                         mode=mode, 
                         **kwargs)
...
def load3yr(energy=True, sirin=False, nopull=False, mode = 'all', **kwargs):
...
def loadIC40(nopull=True, mode = 'box', **kwargs):
    print ("Mode: " + str(mode))
    print ("No pull?" + str(nopull))
    llh=MultiPointSourceLLH()
    print("loading IC40...")
...
def load3yr(sirin=True, nopull=True, mode = 'box', **kwargs):
    print ("Mode: " + str(mode))
    print ("No pull?" + str(nopull))
    llh=MultiPointSourceLLH()
    print("loading IC40...")
...
def load7yr(sirin=False, nopull = False,mode = 'box', **kwargs):
    print ("Mode: " + str(mode))
    llh=MultiPointSourceLLH()
    print("loading IC40...")
    IC40 = init40(mode=mode,nopull=nopull)
    ...
    print("loading IC86I...")
    IC86I = init86I(mode=mode,nopull=nopull)
    print("loading IC86II...")
    IC86II = init86II(mode=mode)
    print("loading IC86III...")
    IC86III = init86II(mode=mode)
    print("loading IC86IV...")
    IC86IV = init86IV(mode=mode)
    samples = [IC40,IC59,IC79,IC86I,IC86II,IC86III,IC86IV]
...
def load7yr_mese(sirin=False,mode='all',**kwargs):
    print ("Mode: " + str(mode))
    llh=MultiPointSourceLLH()
    print("loading IC40...")
    IC40 = init40(mode=mode)
    print("loading IC59...")
    IC59 = init59(mode=mode)
    if sirin:
      print("loading IC79 - sirin's version...")
      IC79 = init79_sirin(mode=mode,decorr=True)
    else:
      print("loading IC79...")
      IC79 = init79(mode=mode,decorr=True)
    print("loading IC86I...")
    IC86I = init86I(mode=mode,decorr=True)
    print("loading IC86II - IC86IV...")
    IC86II_III_IV = init3yr(mode=mode,decorr=True)
    print("loading MESE 3yr...")
    MESE3yr = initMESE3yr(mode=mode)
    print("loading MESE followup...")
    MESEfollowup = initMESEfollowup(mode=mode)
