...
p.add_argument("--ninj", default='0.', type=float,
                help="Number of injected trials.")
...
inj = PointSourceInjector(gamma = gamma, E0 = 1*TeV, seed = args.seed, e_range=(0,np.inf))
...
sens_dir = misc.ensure_dir (outputfolder+'stacking_sensitivity/{0}yr/{1}/{2}/sig/gamma_{3}/{4}/'.format(n,cat,weight,gamma,n_inj))
...
def get_trials():
  t0     = time.time()
  trials = llh.do_trials(args.nscramble, injector=inj, mean_signal=n_inj, src_ra=src_ra, src_dec=src_dec, src_extension = src_ext, src_w = src_w)
  dt     = time.time() - t0
