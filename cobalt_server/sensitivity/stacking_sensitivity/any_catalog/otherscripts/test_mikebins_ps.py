...
bckg_trials = {'n_inj':n_inj,'nsources':np.asarray(nsources), 'TS':np.asarray(TS), 'beta':beta, 'beta_err':beta_err, 'TS_beta':0, 'gamma':np.asarray(gamma)}
...
import data_multi_mikebins as data

## Like in the background trials, we have to define which llhmodel to use.
llh79 = data.init79(energy=True)
llh86I= data.init86I(energy=True)
llh59= data.init59(energy=True)
llh40= data.init40(energy=True)
...
llhmodel = data.multi_init(samples,energy=True)
...
src_dec = np.arcsin(np.linspace(0.5,0.5,1))
...
for i in range(len(src_dec)):
  inj = inj_array[i]
  results = MultiPointSourceLLH.weighted_sensitivity(llhmodel,src_ra=np.atleast_1d(0.),src_dec=np.atleast_1d(src_dec[i]),alpha=.5,beta=.9,inj=inj,trials={'n_inj':[],'TS':[],'nsources':[],'gamma':[]},bckg_trials=bckg_trials,eps=0.03,n_iter=20)
