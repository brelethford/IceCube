...
sensitivity = StackingMultiPointSourceLLH.weighted_sensitivity(llhmodel,src_ra=src_ra,src_dec=src_dec,alpha=norm.sf(5),beta=.5,inj=inj,mc=MC,TSval = TSval,w_theoMC=modelweights['{}'.format(llhweight)],w_theo=modelweights['{}'.format(injweight)],w_theo_fit = modelweights['{}'.format(injweight)],eps=0.02,n_iter=100)
print sensitivity

#choose an output dir, and make sure it exists
this_dir = os.path.dirname(os.path.abspath (__file__))
sens_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/{0}/bstacking_{1}yr/{2}/{3}_inj/discovery/'.format(catalog,str(years),llhweight,injweight))
