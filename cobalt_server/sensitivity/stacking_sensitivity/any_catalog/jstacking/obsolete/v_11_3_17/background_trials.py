...
if catalog:
  if years == 1:
    ...
    bckg_trials = MultiPointSourceLLH.do_trials(llh[0],n_iter = batchsize,src_ra=src_ra,src_dec=src_dec,src_w=modelweights['{}'.format(llhweight)])
else:
  if years == 1:
    bckg_trials = PointSourceLLH.do_trials(llh[0],n_iter = batchsize,src_ra=src_ra,src_dec=src_dec)
  else:
    bckg_trials = MultiPointSourceLLH.do_trials(llh[0],n_iter = batchsize,src_ra=src_ra,src_dec=src_dec)
