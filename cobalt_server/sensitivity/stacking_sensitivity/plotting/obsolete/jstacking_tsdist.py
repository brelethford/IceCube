...
sens = alldata + '{}_inj/sens/'.format(injweight)
disc = alldata + '{}_inj/disc/'.format(injweight)
...
def makehist_inj(datafolder):
  ...
  ##Now we make hists of the test statistics ##
  flux_hist =histlite.hist(TSs,bins=bins,range=range)
  return flux_hist,chi2fit_flux,mu,flux

def printflux(datafolder):
  files = cache.load(datafolder+'gamma2.0.array')
  flux= files[0]['flux']/1000
  mu = files[0]['mu']
  TSval = files[0]['TS']
  return (flux, mu, TSval)

#extract hists for injected trials
hist_sens,chi_sens,mu_sens,flux_sens = makehist_inj(sens)
hist_disc,chi_disc,mu_sens,flux_sens = makehist_inj(disc)
...
for flux_hist, chi2_fit, color in zip(sens_hists,sens_fits, colors):
  histlite.plot1d(ax,flux_hist.normalize(integrate=True),histtype='step',color='red',label='mu = '+str(np.round(printflux(sens)[1][0],2))+', flux = {0:.2E}'.format(Decimal(printflux(sens)[0][0])))
  #plt.axvline(chi2_fit.isf(0.1), color = color, linestyle = ':')

for flux_hist, chi2_fit, color in zip(disc_hists,disc_fits, colors):
  histlite.plot1d(ax,flux_hist.normalize(integrate=True),histtype='step',color='blue',label='mu = '+str(np.round(printflux(disc)[1][0],2))+', flux = {0:.2E}'.format(Decimal(printflux(disc)[0][0])))
