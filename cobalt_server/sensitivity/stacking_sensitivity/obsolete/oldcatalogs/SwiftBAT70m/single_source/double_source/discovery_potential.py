...
datafolder='/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/double_source/dec{0:+010.5}/'.format(dec_deg)
...
## For this experiment I'll try using a 'double source'. If it works, I can assert that there's something wrong in the len(src)=1 case.
src_dec=[np.radians(dec_deg),np.radians(dec_deg)]
src_ra=[0.0,0.0]
...
results = PointSourceLLH.weighted_sensitivity(llh_single,src_ra=src_ra,src_dec=src_dec,alpha=2.867e-7,beta=.5,inj=inj,trials={'n_inj':[],'TS':[],'nsources':[],'gamma':[]},bckg_trials=bckg_trials,eps=0.01,n_iter=250, miniter=2500)# maxtrial=1000)
#Currently have n_iter down from 1000 to reduce estimation time. Also lowering maxtrial from 1000 to 500.
print results

#choose an output dir, and make sure it exists
this_dir = os.path.dirname(os.path.abspath (__file__))
out_dir = misc.ensure_dir (datafolder+'disc/')
