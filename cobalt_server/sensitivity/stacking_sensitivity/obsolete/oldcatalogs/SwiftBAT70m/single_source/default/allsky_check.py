...
def plotretry(datafolder):
    stack_files = [cache.load(datafolder+decfolder+'/sensitivity.array') for decfolder in os.listdir(datafolder) if decfolder.startswith('dec')]
    ##Let's find the declinations used first.
    decfolders = [file for file in os.listdir(datafolder) if file.startswith('dec')]
    dec_stack =  [np.float(decfolder[3:]) for decfolder in decfolders]
    flux_stack = [stack_files[index]['flux'] for index in range(len(stack_files))]
    inj_stack = [stack_files[index]['mu'] for index in range(len(stack_files))]
    sindecrange_stack=np.sin(np.radians(dec_stack))
    TS_stack = [stack_files[index]['TSval'] for index in range(len(stack_files))]
    results_stack = sorted(zip(sindecrange_stack,inj_stack,flux_stack,TS_stack))
    sindecrange_stack,inj_stack,flux_stack,TS_stack = zip(*results_stack)
    return sindecrange_stack, inj_stack, flux_stack, TS_stack

sindec_retry, inj_retry, flux_retry, TS_retry = plotretry('/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/default/')
