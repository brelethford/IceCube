...
# set up submitter
submitter = Submitter (job_dir='/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/multi_year/jobs/{0}_years_{1}'.format (job_id,str(years)))
...
for sin_dec, deg_dec in zip(sindecrange,degdecrange):
  for i in range(batches):
    commands.append('{0} {1} --dec {2} --batch {3} --batchsize {4} --years {5}'.format (env_shell, job_script, sin_dec, i, batchsize, str(years)))
