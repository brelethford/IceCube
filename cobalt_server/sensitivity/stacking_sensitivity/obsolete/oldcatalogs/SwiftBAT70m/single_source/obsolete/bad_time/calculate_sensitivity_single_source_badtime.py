...
llh_single = datatest_stack_badtime.init(energy=True)

mc = datatest_stack_badtime.MC()
...
datafolder='/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/bad_time/dec{0:+010.5}/'.format(dec_deg)
...
results = PointSourceLLH.weighted_sensitivity(llh_single,src_ra=src_ra,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,trials={'n_inj':[],'TS':[],'nsources':[],'gamma':[]},bckg_trials=bckg_trials,eps=0.05,n_iter=250, maxtrial=1000)
