...
datafolder='/data/user/brelethford/Output/stacking_sensitivity/4yr_Starburst/{0}yr/{1}_mhuber_git/background_trials/'.format(str(n),llhweight)
...
llhmodel= new_data_multi.init86I(energy=True,mode='box')
...
sensitivity = StackingPointSourceLLH.weighted_sensitivity(llhmodel,src_ra=src_ra,src_dec=src_dec,alpha=.5,beta=.9,inj=inj,mc=MC,TSval = TSval,w_theoMC=modelweights['{}'.format(llhweight)],w_theo=modelweights['{}'.format(injweight)],w_theo_fit = modelweights['{}'.format(injweight)],eps=0.05,n_iter=100)
print sensitivity
...
sens_dir = misc.ensure_dir ('/data/user/brelethford/Output/stacking_sensitivity/4yr_Starburst/{0}yr/{1}_mhuber_git/{2}_inj/sensitivity/'.format(str(n),llhweight, injweight))
