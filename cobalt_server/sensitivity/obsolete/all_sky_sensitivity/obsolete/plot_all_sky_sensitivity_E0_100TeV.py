...
mc = datatest_stack_bins.MC() #I'll need this to refill the point source injections
...
def plotSensitivity(datafolder_stack_bin,gamma):
    ...
    for i in range(len(inj_stack)):
        inj = ps_injector_stack.PointSourceInjector(gamma, sinDec_bandwidth=.05, src_dec= np.radians(dec_stack)[i],seed=0)
        inj.E0=100
...
sindec_stack, inj_stack, flux_stack = plotSensitivity( '/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/bin_test/', 2.0)
sindec_no_energy, inj_no_energy, flux_no_energy = plotSensitivity( '/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/no_energy/',2.0)
sindec_double, inj_double, flux_double = plotSensitivity('/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/double_source/',2.0)

#And now for gamma=1,3 for regular sensitivity calculation.
sindec_gamma1, inj_gamma1, flux_gamma1 = plotSensitivity( '/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/bin_test/gamma/1.0/',1.0)

sindec_gamma3, inj_gamma3, flux_gamma3 = plotSensitivity( '/data/user/brelethford/Output/all_sky_sensitivity/results/single_stacked/bin_test/gamma/3.0/',3.0)
...
fig_sensitivity.savefig('/data/user/brelethford/AGN_Core/Plots/single_source_sensitivity_100TeV.pdf')
...
ax.set_ylabel(r'$E^2 \cdot \left( \frac{E}{1 \textrm{\small{TeV}}} \right)  ^{\gamma -2} \frac{\partial N}{\partial E} [\frac{TeV}{cm^2 s}]$')
ax.set_xlim(-1.,1.)
#ax.set_ylim(1e-12,1e-10)
ax.semilogy()
ax.grid() 
plt.legend(loc='upper right', prop=propxxsmall)
plt.subplots_adjust (left=.2, bottom=.2)
fig_gamma.savefig('/data/user/brelethford/AGN_Core/Plots/gamma_sensitivity_100TeV.pdf')
